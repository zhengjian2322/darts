...
class AgingEvolution(object):
    def __init__(self, cs, cross_rate, mutation_rate, object_function, pop_size=100,
                 aging_sample_num=10):
        self.cross_rate = cross_rate
        self.mutate_rate = mutation_rate
        self.pop_size = pop_size
        self.object_function = object_function
        self.pop = np.array(cs.sample_configuration(self.pop_size))
        self.aging_sample_num = aging_sample_num
        self.history = np.zeros(self.pop_size)
        self.fitness(range(self.pop_size),2)
    ...
    def fitness(self, candidate_idx, epochs):
        for idx in candidate_idx:
            self.history[idx] = random.random()#self.object_function(self.pop[idx],epochs)
